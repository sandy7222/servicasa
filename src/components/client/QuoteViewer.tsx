...
export const QuoteViewer: React.FC<Props> = ({ order }) => {
  ...
  return <section className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs space-y-3">
    ...
    <div className="rounded-xl border border-teal-200 bg-teal-50 p-3 space-y-1 text-xs"><div className="flex justify-between"><span>Total</span><strong>{formatArs(quote.totalAmount)}</strong></div><div className="flex justify-between text-slate-600"><span>Seña ya acreditada</span><span>-{formatArs(quote.visitDepositCredit)}</span></div><div className="flex justify-between border-t border-teal-200 pt-1 text-sm font-bold text-teal-900"><span>{quote.status === 'accepted' ? 'Saldo' : 'Restante a pagar'}</span>{quote.status === 'accepted' ? <span className="text-emerald-700">Pagado</span> : <span>{formatArs(quote.remainingAmount)}</span>}</div></div>
