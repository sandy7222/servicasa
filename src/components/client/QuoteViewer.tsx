...
export const QuoteViewer: React.FC<Props> = ({ order }) => {
  ...
  return <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-xs space-y-3">
    <div className="flex items-start gap-2"><FileText className="w-4 h-4 mt-0.5 text-teal-700" /><div><h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">Presupuesto del diagnóstico</h3><p className="text-[11px] text-slate-500 dark:text-slate-400">Versión {quote.version}. Los ítems y precios enviados no pueden modificarse.</p></div></div>
    <div className="space-y-1.5">{quote.items.map((item) => <div key={item.id} className="flex justify-between gap-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-xs"><span>🔧 Servicio · {item.description}<span className="block text-[10px] text-slate-500 dark:text-slate-400">{item.quantity} {item.unit} × {formatArs(item.unitPrice)}</span>{item.notes && <span className="mt-1 block text-[10px] text-slate-600 dark:text-slate-400">Nota: {item.notes}</span>}</span><strong className="shrink-0">{formatArs(item.subtotal)}</strong></div>)}</div>
    ...
    <div className="rounded-xl border border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/40 p-3 space-y-1 text-xs"><div className="flex justify-between border-t border-teal-200 dark:border-teal-800 pt-1 text-sm font-bold text-teal-900 dark:text-teal-200"><span>{quote.status === 'accepted' ? 'Total del presupuesto' : 'Total a pagar'}</span>{quote.status === 'accepted' ? <span className="text-emerald-700">Pagado</span> : <span>{formatArs(quote.remainingAmount)}</span>}</div><p className="text-[10px] text-slate-500 dark:text-slate-400 font-normal">La visita de diagnóstico ya pagada es aparte y no se descuenta de este monto.</p></div>
    {isExpired && quote.status === 'sent' && <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-3 text-xs text-amber-900 dark:text-amber-200"><strong>Presupuesto vencido.</strong> Ya no puede aceptarse; podés rechazarlo o solicitar una nueva versión.</div>}
