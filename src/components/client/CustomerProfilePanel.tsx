...
export const CustomerProfilePanel: React.FC = () => {
  ...
  return <section className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-xs space-y-3">
    <div className="flex items-center gap-2"><UserRound className="w-4 h-4 text-teal-600" /><div><h3 className="text-sm font-bold">Mi perfil</h3><p className="text-[11px] text-slate-500 dark:text-slate-400">Solo datos necesarios para identificar tu cuenta.</p></div></div>
    <div className="flex items-center gap-4"><div className="w-16 h-16 rounded-full bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 overflow-hidden flex items-center justify-center font-bold text-teal-700">{avatarPreview ? <img src={avatarPreview} alt="Avatar" className="w-full h-full object-cover" /> : currentUser.avatarText}</div><label className="cursor-pointer text-xs font-bold text-teal-700"><Camera className="inline w-4 h-4 mr-1" />Elegir avatar<input type="file" accept="image/*" className="hidden" onChange={(e) => void uploadAvatar(e.target.files?.[0])} /></label></div>
    <div className="grid sm:grid-cols-2 gap-2"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre y apellido" className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm" /><input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Teléfono (opcional)" className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm" /></div>
    <p className="text-xs text-slate-500 dark:text-slate-400">Correo: <strong>{currentUser.email}</strong></p><button onClick={() => void save()} disabled={!name.trim() || saving} className="inline-flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><Save className="w-3.5 h-3.5" />{saving ? 'Guardando…' : 'Guardar cambios'}</button>
  </section>;
};
