...
export const CustomerAddressesPanel: React.FC = () => {
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-xs space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <MapPin className="w-4 h-4 text-teal-600" />
          <div>
            <h3 className="text-sm font-bold">Mis direcciones</h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">Se editan acá — no afecta a pedidos ya creados.</p>
          </div>
        </div>
        {!editorOpen && (
      ...
      {!editorOpen && addresses.length === 0 && (
        <p className="text-xs text-slate-500 dark:text-slate-400">Todavía no guardaste ninguna dirección.</p>
      )}

      {!editorOpen && addresses.length > 0 && (
        <div className="space-y-2">
          {addresses.map((address) => (
            <div key={address.id} className="flex items-start justify-between gap-2 rounded-lg border border-slate-200 dark:border-slate-700 p-3">
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  {address.label && <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{address.label}</span>}
                  {address.isDefault && (
                    <span className="inline-flex items-center gap-0.5 rounded bg-teal-50 dark:bg-teal-950/40 px-1.5 py-0.5 text-[10px] font-bold text-teal-700 border border-teal-200 dark:border-teal-800">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <button type="button" onClick={() => startEdit(address.id)} className="text-[11px] font-bold text-slate-600 dark:text-slate-400 hover:underline">
                  Editar
                </button>
      ...
      {editorOpen && (
        <div className="space-y-2 rounded-lg border border-teal-200 dark:border-teal-800 bg-teal-50/40 p-3">
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
            Nombre (opcional)
            <input value={labelDraft} onChange={(event) => setLabelDraft(event.target.value)} placeholder="Ej.: Casa, Trabajo" className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm" />
          </label>
          <AddressFields value={addressDraft} onChange={setAddressDraft} showProvince={false} />
          <div className="flex items-center justify-end gap-2 pt-1">
            <button type="button" onClick={cancel} className="px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400">Cancelar</button>
      ...
      {pendingDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60" onClick={() => setPendingDelete(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-5 shadow-2xl border border-slate-200 dark:border-slate-700" onClick={(event) => event.stopPropagation()}>
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">Eliminar dirección</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Los pedidos que ya usaron esta dirección no se ven afectados.</p>
            <div className="flex justify-end gap-2 mt-4">
              <button type="button" onClick={() => setPendingDelete(null)} className="px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400">Cancelar</button>
