...
export function NewClaimModal({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-slate-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={type} onChange={(e) => setType(e.target.value as ClaimType)} className="w-full text-sm px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg">
            ...
            <div>
              ...
              <select value={priority} onChange={(e) => setPriority(e.target.value as ClaimPriority)} className="w-full text-sm px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg">
          ...
          <div>
            ...
            <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Ej: Fuga no resuelta tras reparación" className="w-full text-sm px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg" />
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 flex justify-end gap-2">
            <button onClick={onClose} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg text-xs font-semibold">Cancelar</button>
