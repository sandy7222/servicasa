...
export function ClientFicha({ customerId, onBack }: { customerId: string; onBack: () => void }) {
  ...
  if (!customer) return <main className="max-w-5xl mx-auto p-6"><button onClick={onBack} className="text-teal-700 font-bold">← Volver a clientes</button><p className="mt-6 text-slate-600 dark:text-slate-400">No encontramos este cliente.</p></main>;
  ...
  return <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-5">
    ...
    <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm"><div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><p className="font-mono text-xs text-teal-700">FICHA DE CLIENTE {formatCustomerCode(customer.customerNumber) && `· ${formatCustomerCode(customer.customerNumber)}`}</p><h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 mt-1">{customer.name}</h1><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{customer.phone || 'Sin teléfono'} · {customer.email || 'Sin email'}</p></div><div className="flex items-start gap-3"><div className="grid grid-cols-3 gap-2 text-center"><Metric value={String(customerOrders.length)} label="Órdenes" /><Metric value={money.format(paid)} label="Abonado" /><Metric value={String(warranties.length)} label="Garantías" /></div><EntityActionsMenu
      items={[
        {
          id: 'invite',
    ...
    /></div></div></section>
    <nav className="flex gap-2 overflow-x-auto pb-1">{tabs.map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => setTab(item.id)} className={`whitespace-nowrap inline-flex gap-1.5 items-center px-3 py-2 rounded-lg text-sm font-bold ${tab === item.id ? 'bg-[#0f1b35] text-teal-300' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}><Icon className="w-4 h-4" />{item.label}</button>; })}</nav>
    {tab === 'summary' && <div className="grid md:grid-cols-2 gap-4"><Panel title="Domicilio principal" icon={MapPin}><p className="font-semibold">{customer.address}</p><p className="text-sm text-slate-500 dark:text-slate-400">{customer.neighborhood}</p></Panel><Panel title="Domicilios adicionales" icon={Home}>{addresses.length ? <div className="space-y-3">{addresses.map((address) => <div key={address.id} className="rounded-lg bg-slate-50 dark:bg-slate-950 p-3"><div className="font-bold text-sm">{address.label || 'Domicilio'} {address.is_default && <span className="text-xs text-teal-700">· Predeterminado</span>}</div><div className="text-sm text-slate-600 dark:text-slate-400">{address.address_line}, {address.neighborhood || address.city}</div></div>)}</div> : <p className="text-sm text-slate-500 dark:text-slate-400">No hay domicilios adicionales registrados.</p>}</Panel></div>}
    {tab === 'orders' && <Panel title="Historial de órdenes" icon={ClipboardList}>{customerOrders.length ? <div className="divide-y">{customerOrders.map((order) => <div key={order.id} className="py-3 flex flex-col sm:flex-row justify-between gap-2"><div><div className="font-bold">{order.title}</div><div className="text-sm text-slate-500 dark:text-slate-400">{order.serviceType} · {order.assignedTechnicianName || 'Sin técnico'} · {new Date(order.createdAt).toLocaleDateString('es-AR')}</div></div><div className="self-start flex items-center gap-1.5"><PaymentStatusBadge order={order} size="sm" /><span className={`rounded px-2 py-1 text-xs font-bold ${statusClass(order.status)}`}>{order.status}</span></div></div>)}</div> : <p className="text-slate-500 dark:text-slate-400 text-sm">Sin órdenes registradas.</p>}</Panel>}
    {tab === 'quotes' && <Panel title="Presupuestos enviados" icon={FileText}>{quotes.length ? <div className="space-y-3">{quotes.map(({ quote, order }) => <div key={quote.id} className="rounded-xl border border-slate-200 dark:border-slate-700 p-4"><div className="flex justify-between gap-3"><div><div className="font-bold">{order.title}</div><p className="text-sm text-slate-500 dark:text-slate-400">Versión {quote.version} · {quote.status}</p></div><div className="font-mono font-bold">{money.format(quote.totalAmount)}</div></div><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{quote.status === 'accepted' ? <span className="text-emerald-700 font-semibold">Pagado</span> : `Restante: ${money.format(quote.remainingAmount)}`}</p></div>)}</div> : <p className="text-sm text-slate-500 dark:text-slate-400">No hay presupuestos para este cliente.</p>}</Panel>}
    {tab === 'warranties' && <Panel title="Garantías activas" icon={ShieldCheck}>{warranties.length ? <div className="space-y-3">{warranties.map((order) => { const end = new Date(new Date(order.completedAt!).getTime() + 30 * 86400000); const remaining = Math.max(0, Math.ceil((end.getTime() - Date.now()) / 86400000)); return <div key={order.id} className="rounded-xl bg-emerald-50 border border-emerald-200 p-4"><div className="font-bold text-emerald-900">{order.title}</div><div className="text-sm text-emerald-700">Vence el {end.toLocaleDateString('es-AR')} · quedan {remaining} días</div></div>; })}</div> : <p className="text-sm text-slate-500 dark:text-slate-400">No hay garantías activas.</p>}</Panel>}
    {tab === 'payments' && <Panel title="Historial de pagos" icon={CreditCard}>{payments.length ? <div className="divide-y">{payments.map((payment) => { const order = customerOrders.find((o) => o.id === payment.order_id); return <div key={payment.id} className="py-3 flex justify-between gap-3"><div><div className="font-semibold">{order?.title ?? 'Orden eliminada'}</div><div className="text-xs text-slate-500 dark:text-slate-400">{payment.payment_type} · {paymentMethodLabel(payment.mp_payment_method)}</div><div className="text-xs text-slate-500 dark:text-slate-400">{payment.paid_at ? new Date(payment.paid_at).toLocaleString('es-AR') : new Date(payment.created_at).toLocaleString('es-AR')} · {payment.status}</div></div><div className="font-mono font-bold shrink-0">{money.format(Number(payment.amount))}</div></div>; })}</div> : <p className="text-sm text-slate-500 dark:text-slate-400">No hay pagos registrados para sus órdenes.</p>}</Panel>}
    {tab === 'notes' && <Panel title="Notas internas" icon={StickyNote}><div className="flex gap-2 mb-5"><textarea value={noteText} onChange={(event) => setNoteText(event.target.value)} placeholder="Nota visible solo para administración…" className="flex-1 min-h-20 rounded-lg border border-slate-200 dark:border-slate-700 p-3 text-sm outline-none focus:ring-2 focus:ring-teal-500" /><button disabled={busy || !noteText.trim()} onClick={() => void addNote()} className="self-end inline-flex items-center gap-1 rounded-lg bg-teal-600 text-white font-bold px-3 py-2 disabled:opacity-50"><Plus className="w-4 h-4" />Guardar</button></div>{notes.length ? <div className="space-y-3">{notes.map((note) => <article key={note.id} className="rounded-lg bg-amber-50 dark:bg-amber-950/40 border border-amber-100 p-3"><p className="text-sm text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{note.note}</p><time className="mt-2 block text-xs text-slate-500 dark:text-slate-400">{new Date(note.created_at).toLocaleString('es-AR')}</time></article>)}</div> : <p className="text-sm text-slate-500 dark:text-slate-400">Todavía no hay notas internas.</p>}</Panel>}
    {inviteLinkUrl && (
      <div
...
function Metric({ value, label }: { value: string; label: string }) { return <div className="rounded-lg bg-slate-50 dark:bg-slate-950 px-3 py-2"><div className="font-mono font-black text-slate-900 dark:text-slate-100">{value}</div><div className="text-[10px] uppercase tracking-wide text-slate-500 dark:text-slate-400">{label}</div></div>; }
function Panel({ title, icon: Icon, children }: { title: string; icon: typeof Home; children: ReactNode }) { return <section className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm p-5"><h2 className="font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2 mb-4"><Icon className="w-4 h-4 text-teal-600" />{title}</h2>{children}</section>; }
