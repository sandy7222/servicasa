...
type Payment = { id: string; payment_type: string; status: string; amount: number; currency: string; paid_at: string | null; created_at: string };
...
export function ClientFicha({ customerId, onBack }: { customerId: string; onBack: () => void }) {
  ...
  const loadPrivateData = async () => {
    ...
    const [addressResult, noteResult, paymentResult] = await Promise.all([
      ...
      ids.length ? supabase.from('payment_transactions').select('id, payment_type, status, amount, currency, paid_at, created_at').in('order_id', ids).order('created_at', { ascending: false }) : Promise.resolve({ data: [], error: null }),
    ]);
  ...
  return <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-5">
    ...
    {tab === 'orders' && <Panel title="Historial de órdenes" icon={ClipboardList}>{customerOrders.length ? <div className="divide-y">{customerOrders.map((order) => <div key={order.id} className="py-3 flex flex-col sm:flex-row justify-between gap-2"><div><div className="font-bold">{order.title}</div><div className="text-sm text-slate-500">{order.serviceType} · {order.assignedTechnicianName || 'Sin técnico'} · {new Date(order.createdAt).toLocaleDateString('es-AR')}</div></div><span className={`self-start rounded px-2 py-1 text-xs font-bold ${statusClass(order.status)}`}>{order.status}</span></div>)}</div> : <p className="text-slate-500 text-sm">Sin órdenes registradas.</p>}</Panel>}
    ...
    {tab === 'payments' && <Panel title="Historial de pagos" icon={CreditCard}>{payments.length ? <div className="divide-y">{payments.map((payment) => <div key={payment.id} className="py-3 flex justify-between"><div><div className="font-semibold">{payment.payment_type}</div><div className="text-xs text-slate-500">{payment.paid_at ? new Date(payment.paid_at).toLocaleString('es-AR') : new Date(payment.created_at).toLocaleString('es-AR')} · {payment.status}</div></div><div className="font-mono font-bold">{money.format(Number(payment.amount))}</div></div>)}</div> : <p className="text-sm text-slate-500">No hay pagos registrados para sus órdenes.</p>}</Panel>}
