...
export const TechnicianValidation: React.FC = () => {
  ...
  const load = async () => { const query = supabase.from('technicians').select('id,name,specialty,validation_status').order('name'); const { data } = filter === 'all' ? await query : await query.eq('validation_status', filter); setRows((data ?? []) as Row[]); };
