...
export function ClientsTable({ onOpen }: { onOpen: (customerId: string) => void }) {
  ...
  return <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
    ...
    <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <label className="relative block max-w-xl flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Buscar por nombre, email o teléfono…" className="w-full rounded-lg border border-slate-200 dark:border-slate-700 pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-teal-500" /></label>
        <label className="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300"><input type="checkbox" checked={onlyWarranty} onChange={(event) => { setOnlyWarranty(event.target.checked); setPage(1); }} className="accent-teal-600" />Con garantía activa</label>
      </div>
      <div className="overflow-x-auto"><table className="min-w-[960px] w-full text-left text-sm"><thead className="sticky top-0 bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide"><tr>{([['name', 'Cliente'], ['orders', 'Órdenes'], ['spent', 'Total abonado'], ['warranties', 'Garantías'], ['lastOrder', 'Última orden']] as [SortKey, string][]).map(([key, label]) => <th key={key} className="px-4 py-3"><button onClick={() => order(key)} className="font-bold hover:text-teal-700">{label}{indicator(key)}</button></th>)}<th className="px-4 py-3">Contacto</th><th className="px-4 py-3 text-right">Acción</th></tr></thead>
        <tbody>{visible.map((row) => <tr key={row.customer.id} onClick={() => onOpen(row.customer.id)} className="border-t border-slate-100 dark:border-slate-800 cursor-pointer hover:bg-teal-50/60 transition"><td className="px-4 py-3"><div className="flex items-center gap-1.5"><span className="font-mono text-[10px] font-bold text-teal-700 bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 rounded px-1 py-0.5">{formatCustomerCode(row.customer.customerNumber) ?? '—'}</span><div className="font-bold text-slate-900 dark:text-slate-100">{row.customer.name}</div></div><div className="text-xs text-slate-500 dark:text-slate-400">{row.customer.address} · {row.customer.neighborhood}</div></td><td className="px-4 py-3"><strong>{row.totalOrders}</strong><span className="text-slate-500 dark:text-slate-400"> / {row.completedOrders} cerradas</span></td><td className="px-4 py-3 font-mono font-semibold">{money.format(row.totalSpent)}</td><td className="px-4 py-3">{row.warranties ? <span className="inline-flex items-center gap-1 text-emerald-700 font-semibold"><ShieldCheck className="w-4 h-4" />{row.warranties} activa{row.warranties > 1 ? 's' : ''}</span> : <span className="text-slate-400">—</span>}</td><td className="px-4 py-3">{row.lastOrder ? new Date(row.lastOrder).toLocaleDateString('es-AR') : '—'}</td><td className="px-4 py-3"><div>{row.customer.phone || '—'}</div><div className="text-xs text-slate-500 dark:text-slate-400">{row.customer.email || '—'}</div></td><td className="px-4 py-3 text-right"><button onClick={(event) => { event.stopPropagation(); onOpen(row.customer.id); }} className="inline-flex items-center gap-1 text-teal-700 font-bold hover:underline">Abrir ficha <ExternalLink className="w-3.5 h-3.5" /></button></td></tr>)}{visible.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-slate-500 dark:text-slate-400">No encontramos clientes con esos filtros.</td></tr>}</tbody></table></div>
      <footer className="p-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-sm"><span className="text-slate-500 dark:text-slate-400">{filtered.length} cliente{filtered.length === 1 ? '' : 's'}</span><div className="flex gap-2"><button disabled={page === 1} onClick={() => setPage((value) => value - 1)} className="px-3 py-1.5 rounded border disabled:opacity-40">Anterior</button><span className="px-2 py-1.5">{page} / {pages}</span><button disabled={page === pages} onClick={() => setPage((value) => value + 1)} className="px-3 py-1.5 rounded border disabled:opacity-40">Siguiente</button></div></footer>
    </section>
  </main>;
}
