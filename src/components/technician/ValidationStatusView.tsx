...
export function ValidationStatusView() {
  ...
  return <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-2"><div className="flex items-center gap-2"><Bell className="w-4 text-teal-600" /><div><h2 className="text-sm font-bold">Estado de validación</h2><p className="text-[11px] text-slate-500 dark:text-slate-400">Completá o corregí lo observado para recibir trabajos.</p></div></div>{pending.length ? pending.map((r) => <div key={r.requirement_type} className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-2 text-xs text-amber-950"><CircleAlert className="inline w-3.5 mr-1" /><b>{labels[r.requirement_type]}</b>: {r.review_notes || 'Pendiente de revisión.'}</div>) : <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-800"><CheckCircle2 className="inline w-3.5 mr-1" />No tenés requisitos obligatorios pendientes.</div>}{notifications.map((n) => <div key={n.id} className="rounded-lg bg-slate-50 dark:bg-slate-950 p-2 text-xs"><b>{n.title}</b><p className="text-slate-600 dark:text-slate-400">{n.message}</p></div>)}</section>;
}
