...
export const ProfessionalProfile: React.FC = () => {
  ...
  const uploadDocument = async (type: string, file?: File) => {
    if (!file) return;
    if (!['application/pdf', 'image/jpeg', 'image/png'].includes(file.type) || file.size > 10 * 1024 * 1024) { showToast('El documento debe ser PDF, JPG o PNG, y pesar hasta 10 MB.', 'warning'); return; }
    ...
    if (error) { showToast('No se pudo subir el documento.', 'error'); return; }
  ...
  return <div className="max-w-4xl mx-auto space-y-4 py-4">
    ...
    {profile && <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-4"><div className="flex items-center justify-between"><h2 className="font-bold text-sm">Perfil público</h2><span className={`text-[11px] font-bold rounded-full px-2 py-1 ${profile.validation_status === 'approved' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 dark:bg-amber-950/40 text-amber-700'}`}>{statusCopy[profile.validation_status] ?? profile.validation_status}</span></div>
      <div className="flex gap-4 items-center"><div className="w-16 h-16 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 flex items-center justify-center font-bold text-slate-500 dark:text-slate-400">{avatarUrl ? <img src={avatarUrl} alt="Foto profesional" className="w-full h-full object-cover" /> : technician.name.slice(0, 2).toUpperCase()}</div><label className="cursor-pointer text-xs font-bold text-teal-700"><Camera className="inline w-4 mr-1" />Cambiar foto<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => void uploadAvatar(e.target.files?.[0])} /></label></div>
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
          Nivel de formación
          <select value={profile.education_level} onChange={(e) => setProfile({ ...profile, education_level: e.target.value })} className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 p-2 text-sm font-normal">
      ...
      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
        ...
        <textarea value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} rows={3} className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 p-2 text-sm font-normal resize-none" />
      </label>
      <p className="text-[11px] text-slate-500 dark:text-slate-400"><Building2 className="inline w-3.5 mr-1 text-teal-600" />Estos datos los revisa administración antes de habilitarte — mantenelos actualizados.</p>
      {profile.validation_notes && <p className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-3 text-xs text-amber-800"><CircleAlert className="inline w-4 mr-1" />{profile.validation_notes}</p>}<button onClick={() => void saveProfile()} disabled={saving} className="inline-flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-60"><Save className="w-3.5" />Guardar perfil</button></section>}
