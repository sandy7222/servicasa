...
import { getCategoriesByRubro, getRubros, type CatalogCategory, type ServiceRubro } from '../../lib/catalog';
...
const slugForServiceType = (serviceType: string) => {
  const value = serviceType.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
...
export const QuoteBuilder: React.FC<Props> = ({ order }) => {
  const { currentUser, refreshRemoteData, showToast } = useApp();
  ...
  return <section className="space-y-3">
    ...
    {quote && <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-2"><div className="flex justify-between border-b pb-2 text-sm"><strong>Detalle presupuestado</strong><strong>{formatArs(totals.total)}</strong></div>{quote.items.map((item) => <div key={item.id} className="rounded-lg bg-slate-50 p-2.5 text-xs"><div className="flex items-start justify-between gap-2"><span><strong>{item.description}</strong><span className="mt-0.5 block text-[10px] text-slate-500">Precio publicado: {formatArs(item.unitPrice)} por {item.unit}</span></span>{isDraft && <button type="button" disabled={busy} onClick={() => void deleteItem(item.id)} aria-label="Eliminar ítem" className="text-rose-600"><Trash2 className="h-4 w-4" /></button>}</div>{isDraft && <div className="mt-2 flex items-center gap-2"><button type="button" disabled={busy || item.quantity <= 1} onClick={() => void updateItem(item.id, { quantity: item.quantity - 1 })} className="rounded border border-slate-300 p-1"><Minus className="h-3 w-3" /></button><span className="min-w-8 text-center font-bold">{item.quantity}</span><button type="button" disabled={busy} onClick={() => void updateItem(item.id, { quantity: item.quantity + 1 })} className="rounded border border-slate-300 p-1"><Plus className="h-3 w-3" /></button><input defaultValue={item.notes ?? ''} onBlur={(event) => { if (event.target.value !== (item.notes ?? '')) void updateItem(item.id, { notes: event.target.value.trim() || null }); }} placeholder="Nota opcional" className="min-w-0 flex-1 rounded border border-slate-200 px-2 py-1 text-[11px]" /></div>}{item.notes && !isDraft && <p className="mt-1 text-[10px] text-slate-600">Nota: {item.notes}</p>}<strong className="mt-2 block text-right">{formatArs(item.subtotal)}</strong></div>)}<div className="flex justify-between text-xs text-slate-600"><span>Seña acreditada</span><span>-{formatArs(totals.deposit)}</span></div><div className="flex justify-between border-t pt-2 text-sm font-bold text-teal-800"><span>Restante</span><span>{formatArs(totals.remaining)}</span></div></div>}
