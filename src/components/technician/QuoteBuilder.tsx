...
export const QuoteBuilder: React.FC<Props> = ({ order }) => {
  ...
  return <section className="space-y-3">
    ...
    {!isDraft ? <div className="rounded-xl border border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/40 p-4 text-xs text-teal-900 dark:text-teal-200"><strong>Presupuesto enviado.</strong> El cliente ya puede revisarlo desde su portal. No podés cambiarlo.</div> : <>
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-3"><div className="flex items-center justify-between gap-2"><h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">Servicios publicados</h4><select value={rubroId} onChange={(event) => setRubroId(event.target.value)} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1.5 text-xs">{rubros.map((rubro) => <option key={rubro.id} value={rubro.id}>{rubro.icon} {rubro.name}</option>)}</select></div>{loadingCatalog ? <p className="text-xs text-slate-500 dark:text-slate-400">Cargando catálogo…</p> : <div className="grid gap-2 sm:grid-cols-2">{categories.map((category) => { const selected = quote?.items.find((item) => item.categoryId === category.id); return <button key={category.id} type="button" disabled={busy} onClick={() => void addCategory(category)} className={`rounded-lg border p-3 text-left transition disabled:opacity-60 ${selected ? 'border-teal-500 bg-teal-50 dark:bg-teal-950/40' : 'border-slate-200 dark:border-slate-700 hover:border-teal-300'}`}><span className="block text-xs font-bold text-slate-900 dark:text-slate-100">{category.name}</span><span className="mt-1 block text-[10px] text-slate-500 dark:text-slate-400">{category.description}</span><span className="mt-2 block text-xs font-black text-teal-800">{formatArs(category.basePrice)} por {category.unit}</span>{selected && <span className="mt-1 block text-[10px] font-bold text-teal-700">Agregado: {selected.quantity} {selected.unit}</span>}</button>; })}</div>}</div>

      {(loadingTarifario || tarifario.length > 0) && <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-3">
        <div className="flex items-center justify-between gap-2"><h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">Tarifario completo ({tarifario.length} ítems)</h4></div>
        <input value={tarifarioSearch} onChange={(event) => setTarifarioSearch(event.target.value)} placeholder="Buscar por nombre…" className="w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-xs" />
        {loadingTarifario ? <p className="text-xs text-slate-500 dark:text-slate-400">Cargando tarifario…</p> : <div className="max-h-80 space-y-4 overflow-y-auto pr-1">
          {tarifarioGroups.length === 0 && <p className="text-xs text-slate-500 dark:text-slate-400">Sin resultados para "{tarifarioSearch}".</p>}
          {tarifarioGroups.map((group) => <div key={group.id ?? 'sin-subcategoria'}>
            ...
            <div className="space-y-1">{group.items.map((item) => { const selected = quote?.items.find((qi) => qi.serviceId === item.id); return <button key={item.id} type="button" disabled={busy} onClick={() => void addTarifarioItem(item)} className={`flex w-full items-center justify-between gap-2 rounded-lg border p-2 text-left transition disabled:opacity-60 ${selected ? 'border-teal-500 bg-teal-50 dark:bg-teal-950/40' : 'border-slate-200 dark:border-slate-700 hover:border-teal-300'}`}><span className="text-[11px] text-slate-800 dark:text-slate-200">{item.name}{selected && <span className="ml-1 font-bold text-teal-700">· x{selected.quantity}</span>}</span><span className="shrink-0 text-[11px] font-black text-teal-800">{formatArs(item.price)}</span></button>; })}</div>
          </div>)}
        </div>}
      </div>}
    ...
    {quote && <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-2"><div className="flex justify-between border-b pb-2 text-sm"><strong>Detalle presupuestado</strong><strong>{formatArs(totals.total)}</strong></div>{quote.items.map((item) => <div key={item.id} className="rounded-lg bg-slate-50 dark:bg-slate-950 p-2.5 text-xs"><div className="flex items-start justify-between gap-2"><span><strong>{item.description}</strong>{item.itemType === 'material' && <span className="ml-1.5 rounded bg-amber-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-amber-800">Repuesto</span>}<span className="mt-0.5 block text-[10px] text-slate-500 dark:text-slate-400">Precio publicado: {formatArs(item.unitPrice)} por {item.unit}</span></span>{isDraft && <button type="button" disabled={busy} onClick={() => void deleteItem(item.id)} aria-label="Eliminar ítem" className="text-rose-600"><Trash2 className="h-4 w-4" /></button>}</div>{isDraft && <div className="mt-2 flex items-center gap-2"><button type="button" disabled={busy || item.quantity <= 1} onClick={() => void updateItem(item.id, { quantity: item.quantity - 1 })} className="rounded border border-slate-300 p-1"><Minus className="h-3 w-3" /></button><span className="min-w-8 text-center font-bold">{item.quantity}</span><button type="button" disabled={busy} onClick={() => void updateItem(item.id, { quantity: item.quantity + 1 })} className="rounded border border-slate-300 p-1"><Plus className="h-3 w-3" /></button><input defaultValue={item.notes ?? ''} onBlur={(event) => { if (event.target.value !== (item.notes ?? '')) void updateItem(item.id, { notes: event.target.value.trim() || null }); }} placeholder="Nota opcional" className="min-w-0 flex-1 rounded border border-slate-200 dark:border-slate-700 px-2 py-1 text-[11px]" /></div>}{item.notes && !isDraft && <p className="mt-1 text-[10px] text-slate-600 dark:text-slate-400">Nota: {item.notes}</p>}<strong className="mt-2 block text-right">{formatArs(item.subtotal)}</strong></div>)}<div className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400"><span>Mano de obra: {formatArs(totals.labor)}</span><span>Repuestos: {formatArs(totals.materials)}</span></div><div className="flex justify-between border-t pt-2 text-sm font-bold text-teal-800"><span>{quote.status === 'accepted' ? 'Saldo' : 'Restante'}</span>{quote.status === 'accepted' ? <span className="text-emerald-700">Pagado</span> : <span>{formatArs(totals.remaining)}</span>}</div><p className="text-[10px] text-slate-500 dark:text-slate-400 font-normal">La visita de diagnóstico ya pagada es aparte y no se descuenta de este monto.</p></div>}
