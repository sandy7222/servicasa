...
const FIELD_CLASS =
  'w-full px-3.5 py-2.5 rounded-lg border border-slate-300 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent';
    ...
    >
      {submitted ? (
        <div className="flex flex-col items-center text-center gap-3 py-6">
          ...
          <p className="font-bold text-slate-900 dark:text-slate-100">¡Consulta enviada!</p>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Gracias por contactarnos. Nos comunicaremos contigo a la brevedad.
          </p>
          <button
            onClick={handleClose}
            className="mt-2 px-5 py-2.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-semibold text-sm transition-colors"
          >
            Cerrar
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Nombre de empresa</label>
            <input required value={form.companyName} onChange={set('companyName')} className={FIELD_CLASS} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Persona de contacto</label>
            <input required value={form.contactName} onChange={set('contactName')} className={FIELD_CLASS} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Correo</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tipo de servicio</label>
            <input
              required
