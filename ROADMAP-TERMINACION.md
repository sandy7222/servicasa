...
- [x] Probar explícitamente que el trigger siga rechazando un precio manipulado, contra el catálogo real, después de la migración. **Cerrado 1/9:** `supabase/tests/pricing_triggers.sql` toma un servicio activo y un cliente reales dentro de `BEGIN`/`ROLLBACK`, intenta insertar importe fijo y seña manipulados, y comprueba que el servidor los reemplaza por `services.price × cantidad` y `system_settings.visit_deposit_amount`. La consulta falla si alguna aserción no se cumple.
...
- [ ] Quedan scripts auxiliares y seeds en `supabase/sql/` por clasificar/formalizar. **Actualización 1/9:** el esquema sí reconstruye desde cero con las 40 migraciones actuales (CI `supabase db start` aprobado); lo que falta para una reproducción completa de producto es un seed único, seguro e idempotente del catálogo y de datos de desarrollo, no el esquema.
...
- [x] `vercel.json`, automatización de CI y pruebas automatizadas existen. **Actualización 1/9:** CI bloquea lint, build, Vitest con cobertura y reconstrucción de Supabase; hay 106 pruebas Vitest y 4 specs E2E (la capa autenticada sigue parcial hasta cargar secretos E2E aislados).
...
- [ ] Regenerar tipos TypeScript de Supabase y reemplazar gradualmente tipos manuales inseguros. **Avance 1/9:** `src/types/database.types.ts` fue generado desde el schema `public` live y quedó versionado, con el comando de regeneración documentado en su cabecera. La adopción en el cliente se deja gradual para no romper los tipos manuales existentes de una vez.
- [x] **Capa de pruebas Supabase (decisión: rollback-safe, no pgTAP):** los 11 scripts de `supabase/tests/*.sql` pasan contra live dentro de `BEGIN`/`ROLLBACK`, incluidos conversaciones, liquidaciones, reclamos, settings y ambos triggers de precio. Las fixtures/aserciones obsoletas se actualizaron sin cambiar políticas ni reglas de negocio. Pendiente de calidad posterior: centralizar un runner que convierta automáticamente un resultado interno `false` en código de salida no cero.
...
- [ ] Una base local o de staging vacía se reconstruye solo con migraciones y seeds documentados. **Parcial, actualizado 1/9:** la reconstrucción del esquema actual con las 40 migraciones se ejecuta y pasa en GitHub CI (`supabase db start`). Falta documentar/centralizar el seed idempotente y seguro del catálogo/datos de desarrollo antes de marcar la reproducción completa.
...
- [x] Los tests RLS positivos y negativos pasan. **Verificado 1/9:** 11 scripts rollback-safe ejecutados contra live con todas las comprobaciones actuales verdaderas, incluidos los casos negativos de cliente/técnico ajeno, configuración, mensajes, reclamos, liquidaciones y precio manipulado. Queda como mejora del runner convertir un eventual `false` futuro en fallo de proceso automático.
- [x] Los tipos generados están versionados. **Cerrado 1/9:** `src/types/database.types.ts` refleja el schema público live; se regenerará después de cada cambio de schema antes de adoptar más consultas tipadas.
...
- [x] Definir ventana de garantía en configuración, no hardcodeada — **cerrado en Fase 7:** `warranty_days` vive en `system_settings`, `customer_summary` lo lee dinámicamente y `system_settings_rls.sql` verifica ambos casos (1 y 30 días) dentro de `ROLLBACK`.
...
> **Actualización de cierre por capas (31/8-2/9):** se instaló cobertura V8 y se separaron comandos para unitarios, API, componentes y E2E. La batería Vitest creció a **15 archivos / 106 pruebas** y pasa completa. La capa API agrega autorización por rol, montos calculados en servidor, propiedad de presupuesto/dirección, cancelación ante error de proveedor e idempotencia del webhook. Los **11 scripts SQL rollback-safe** pasan contra Supabase live sin persistir datos, incluido el guard de precio/seña (`supabase/tests/pricing_triggers.sql`). E2E registra **4 specs / 9 escenarios**: 2 pasan sin credenciales privadas y 7 autenticados se omiten de forma explícita hasta cargar los secretos del entorno de CI. El CI reproducible pasó completo el 2/9 (incluidas migraciones desde cero). Informe reproducible: `docs/audits/2026-08-31-phase8.md`.
...
- [~] **Componentes (31/8, parcial verificado):** 3 archivos / 17 pruebas (`NotificationBell`, `SystemSettingsPanel`, `QuoteViewer`). Ya cubre estados de presupuesto, vencimiento, aceptación/rechazo y entrega correcta al pago; todavía falta ampliar formularios/estados de carga y error del resto del producto.
- [x] **Supabase (2/9):** se mantiene la decisión de usar los 11 scripts rollback-safe de `supabase/tests/*.sql` como capa de integración en vez de pgTAP. Los 11 fueron ejecutados contra Supabase live y pasaron con `ROLLBACK`, incluido el guard de precio y seña; se actualizaron únicamente fixtures/aserciones obsoletas, no políticas ni reglas.
...
- [~] **E2E (2/9, parcial verificado):** 4 specs / 9 escenarios registrados. Sin secretos privados pasan credenciales inválidas y el handoff de checkout invitado; 7 escenarios autenticados se omiten explícitamente. Incluye el spec de intentos cruzados, pero no se marca como flujo cerrado hasta ejecutarlo con cuentas E2E aisladas. Para cerrar la capa faltan esos secretos y los flujos transaccionales restantes.
...
- [x] No hay advertencias de seguridad Supabase de prioridad alta sin decisión documentada — los Advisors revisados el 2/9 reportaron **0 ERROR**. Se cerraron o documentaron `technicians`, `is_admin`, tokens de invitación y `technician_public_view`; las advertencias de rendimiento de RLS (`auth_rls_initplan`/`multiple_permissive_policies`) son WARN/INFO evaluados, no fugas.
...
> **Segunda actualización (2/9/2026, auditoría de reproducibilidad pedida por Sandy):** el bloqueante real de la Primera actualización ("24 migraciones aplicadas en la base, solo 6 archivos en el repo") está **resuelto**. Conteo independiente hoy contra el proyecto real (`ayszrtieplmqscqtabsu`): `list_migrations` devuelve 40 migraciones aplicadas; `supabase/migrations/*.sql` tiene exactamente 40 archivos; los 40 timestamps+nombres coinciden 1 a 1 en ambas direcciones (0 aplicadas sin archivo local, 0 archivos locales sin aplicar). Esto reconfirma hoy, de forma independiente, el hallazgo de la Reauditoría de Codex del 30/8 (que ya reportaba 40/40) — la reproducibilidad del esquema no se degradó con el trabajo de la sesión de liquidación de visita/rediseño de dirección/`technician_coverage_areas`, se mantuvo intacta. Siguen sin resolver los demás puntos de la Primera actualización (backup real con `pg_dump`, confirmación de los dashboards de Auth y del webhook de Mercado Pago, smoke test completo re-corrido) — el lanzamiento sigue sin estar listo por esos motivos, ya no por las migraciones.

> **Tercera actualización (2/9/2026, pasada completa de release manager, pedido explícito de Sandy):** avance real en varios frentes, con un freno de seguridad respetado en vez de esquivado. Detalle completo en `docs/fase10-checklist.md` (Actualización 2/9); resumen:
> - [x] Automatizado: `tsc`/`vitest` (106/106)/`build` limpios; Advisors seguridad **0 ERROR**; Advisors rendimiento **0 ERROR** (68 WARN + 52 INFO, todos cosméticos); migraciones 40/40 (ver Segunda actualización arriba).
> - [x] **Backup nuevo y real:** `pg_dump` completo contra producción hoy (`backups/pg_dump_2026_09_02.sql`, 502KB), verificado estructuralmente contra el esquema en vivo (51 tablas = 51 tablas, con datos reales).
> - [ ] **Restauración:** sigue sin poder probarse — se intentó instalar un servidor Postgres local desechable (sin Docker) y se confirmó que la instalación del 28/8 fue deliberadamente solo de cliente (`pg_dump`/`psql`, sin `initdb` funcional). No se instaló el servidor completo sin confirmación de Sandy, por ser infraestructura nueva.
> - [ ] **Contraseña de prueba `TecniUrbano2026!`** (hallazgo del 30/8): un intento de login con esas credenciales contra el admin real falló esta sesión — puede que el hallazgo ya esté obsoleto, sin confirmar a propósito.
> - [~] **Smoke test:** se logró un login real contra `tecniurbano.online` con una sesión de admin autenticada (magic link vía `auth.admin.generate_link`, mismo mecanismo que la Fase 9) — confirma landing/catálogo y el mecanismo de login/auth completo de punta a punta en producción. Al navegar al panel de Admin Hub para seguir con notificaciones/liquidaciones/reclamos/exportación, el clasificador de seguridad de Auto Mode bloqueó la acción — se respetó el freno en vez de buscar la forma de saltarlo, tal como pidió Sandy ("detenete antes de cualquier paso irreversible"). Quedan 8 de los 10 flujos del smoke test sin re-correr hoy.
> - [x] Rollback (`docs/rollback.md`): revisado completo, sigue preciso.
> - **Pendiente de decisión de Sandy:** instalar servidor Postgres local completo para probar restauración (sí/no); confirmar o rotar la contraseña de prueba; hacer él mismo el resto del smoke test contra producción, o autorizar explícitamente que se reintente el panel de admin.

> **Cuarta actualización (2/9/2026, cierre de dos de los tres pendientes que Sandy autorizó explícitamente):** detalle completo en `docs/fase10-checklist.md` (Actualización 2/9, quinta pasada); resumen:
> - [x] **Restauración — resuelta con evidencia de punta a punta.** Servidor Postgres local desechable vía la distribución portable de EDB (sin instalador, sin servicio de Windows, sin elevación). Restauré `backups/pg_dump_2026_09_02.sql`: **51/51 tablas**, datos reales confirmados (filas reconocibles de `customers` y las dos `service_orders` de prueba ya conocidas de esta sesión). Los únicos huecos del restore son esperables (roles y esquema `auth` de Supabase, ausentes en Postgres vanilla; no aplican a una restauración real contra un proyecto Supabase). Footprint borrado al terminar. Quedan dos procesos `postgres.exe` huérfanos que no pude terminar por permiso denegado — no representan riesgo de datos, pero Sandy los tiene que cerrar manualmente desde el Administrador de tareas si quiere liberar el puerto 5544.
> - [x] **Contraseña de prueba — rotada.** Nueva contraseña generada y aplicada a las 4 cuentas reales vía la API admin de Supabase, verificada con un login real contra Supabase. Actualizada en los 5 archivos que la referencian como valor vivo (`agent.md`, `README.md`, `playwright.config.ts`, `src/lib/supabaseData.ts`, `src/views/AuthView.tsx`); las menciones históricas fechadas en este roadmap y en `CHANGELOG.md` se dejaron intactas a propósito. `tsc`/`vitest` (106/106)/`build` limpios después del cambio. Nueva contraseña: `TecnilFV2ly3Z!21` (también anotada en el checklist).
> - **[~] Smoke test:** en curso, a cargo de Sandy — va a hacer él mismo el click-through de los 8 flujos restantes contra `tecniurbano.online`. No se reintentó el panel de admin por instrucción explícita.
...
| 1. Supabase reproducible | 🟡 En curso — esquema reconstruible, RLS verde y tipos versionados; faltan seed completo y clasificación de scripts auxiliares | 23/8/2026 |  | `main` | 1/9: `migration list --linked` 40 local = 40 remoto; CI aprobó `supabase db start`; 11 suites rollback-safe live pasan; `src/types/database.types.ts` generado desde live; falta seed idempotente integral y GRANT/Data API. **Reconfirmado 2/9** con conteo independiente: sigue 40 aplicadas = 40 archivos, 1 a 1, tras toda la sesión de seña/dirección/`technician_coverage_areas` |
| 2. Reclamos y garantías | 🟡 Casi — falta atomicidad en resolución y evidencia técnica adjunta | 23/8/2026 |  | `feature/mercadopago-payments-backend` | Migración `connect_support_cases_module`; `ClaimDetail.tsx`, `MyClaimsPanel.tsx`; `supabase/tests/support_cases_rls.sql` (7/7 OK); garantía ahora configurable mediante `warranty_days` (Fase 7) |
...
| 8. Calidad y CI | 🟡 Parcial sólido — base aprobada | 25/8/2026 | 2/9/2026 | commits `9cc8467`, `69ab7f8`, `535ffbd`, `156152e` | CI verde: lint, build, 106 Vitest, seguridad y reconstrucción de Supabase; 11 SQL rollback-safe live; 0 ERROR en Advisors. Quedan E2E autenticados, más componentes y rate limiting |
| 9. Staging y Vercel | ✅ Hecho, con riesgo aceptado documentado | 26/8/2026 | 27/8/2026 | `main` | Preview protegido, rollback documentado, recuperación de contraseña probada de punta a punta y CI activo; Preview/Production comparten Supabase por decisión explícita de Sandy (riesgo aceptado, no un olvido); faltan alertas, smoke test automático completo y confirmación de dashboards de Auth — quedan para una sesión futura, no bloquean el cierre declarado el 27/8 |
| 10. Lanzamiento | 🟡 En curso — no listo para lanzar | 27/8/2026 |  | `main` | `docs/fase10-checklist.md` (checklist inicial 27-28/8, pasada completa de release manager 2/9, actualización de evidencia 3/9); bloqueante de migraciones resuelto (40/40, 1 a 1); backup y restauración real 2/9; contraseña de prueba rotada 2/9; login/auth confirmado contra `tecniurbano.online`; **caso Sandbox completo de María Rodríguez (aire acondicionado) confirmado por auditoría de datos 3/9:** seña, saldo, orden completada y liquidaciones. Pendientes concretos: recorrido de cliente registrado, presupuesto/firma, reclamo/mensaje, notificación, visualización de liquidación y exportación administrativa. |
