...
| 10-extra. Bug de orden de cliente logueado creada antes del pago | 🟡 Código desplegado y verificado en partes — falta que Sandy complete un pago real de prueba para cerrar del todo | 28/8/2026 |  | `main`, commit `0e94d6f` | Mismo bug que 1.1-extra (guest), esta vez en "Solicitar diagnóstico"/"Sé qué trabajo necesito" para cliente logueado — casos reales `432efd32-...`, `bb802ded-...`. Corregido con el mismo patrón (`customer_order_drafts` + `api/orders/request-service.ts` + `api/orders/pending-draft.ts` + `api/payments/retry-draft.ts` + rama nueva en `api/payments/webhook.ts`); se eliminó además la política RLS `service_orders_insert_customer_request` que permitía recrear el bug llamando directo a la API REST de Supabase, sin pasar por la app. Verificado en producción real: `POST /api/orders/request-service` crea el borrador y la preferencia de MP sin tocar `service_orders` (confirmado por SQL: cero filas). Se agregó también la generación real de notificaciones `payment_pending`/`payment_approved` (no existía ninguna lógica que las creara antes, pese a que el tipo ya estaba en el enum) y la resolución de `entity_type='payment'` en `CustomerView.tsx` cuando apunta a un borrador. **Pendiente real:** completar un pago de punta a punta con la tarjeta de prueba de Mercado Pago (Mastercard 5031 7557 3453 0604, titular `APRO`, venc. 11/30, CVV 123, DNI 12345678) para confirmar que el webhook crea la orden — el checkout de MP usa iframes de "secure fields" que bloquean el tipeo automatizado (protección anti-bot esperable en un campo de tarjeta), así que ese último paso necesita un click real de Sandy, no de Code.
